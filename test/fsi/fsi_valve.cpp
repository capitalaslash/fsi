...
int main (int argc, char** argv)
{
    ...
    GetPot param_file("param_fsi_valve.dat");
    ...
    // INLET/OUTLET
    system_dy .get_dof_map().add_dirichlet_boundary( DirichletBoundary( {1,3}, {dy_var}, ZeroFunction<Real>() ) );
    system_vel.get_dof_map().add_dirichlet_boundary( DirichletBoundary( {1,3}, {u_var},  ZeroFunction<Real>() ) );

    // WALL
    system_dx .get_dof_map().add_dirichlet_boundary( DirichletBoundary( {2}, {dx_var}, ZeroFunction<Real>() ) );
    system_dy .get_dof_map().add_dirichlet_boundary( DirichletBoundary( {2}, {dy_var}, ZeroFunction<Real>() ) );
    system_vel.get_dof_map().add_dirichlet_boundary( DirichletBoundary( {2}, {u_var,v_var},  ZeroFunction<Real>() ) );

    // SYMMETRY
    system_dx .get_dof_map().add_dirichlet_boundary( DirichletBoundary( {4}, {dx_var}, ZeroFunction<Real>() ) );
    system_vel.get_dof_map().add_dirichlet_boundary( DirichletBoundary( {4}, {u_var},  ZeroFunction<Real>() ) );
    ...
    es.parameters.set<std::string>("output_dir") = param_file("output_dir", "output_valve/");
    es.parameters.set<std::string>("basename") = param_file("basename", "fsivalve");
