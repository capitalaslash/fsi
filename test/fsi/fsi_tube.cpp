...
struct ExtPressure
{
    explicit ExtPressure(Real const p):
        p_max(p)
    {}
    ...
    {
        // pulsating
        //return p_max*0.5*(1.-std::cos(M_PI*4.*t));
...
void generate_tube( Mesh & mesh, GetPot & param )
{
    uint const flag_f = param("flag_f", 11);
    uint const flag_s = param("flag_s", 12);
    uint const nx_f = param("nx_f", 10);
    uint const nx_s = param("nx_s", 2);
    uint const nx = nx_f + nx_s;
    uint const ny = param("ny", 8);
    Real const ox = param("ox", 0.0);
    Real const lx = param("lx", 1.1);
    Real const oy = param("oy", 0.0);
    Real const ly = param("ly", 10.0);
    ...
    const Real f = param("mesh_scaling", 0.9);
    const uint np = 2*nx+1;
    const Real h_f = (li-ox)*(1-f)/(1-std::pow(f,2*nx_f));
    const Real h_s = (lx-li) / (2*nx_s);
    ...
    {
        Node* node = *nd;
        const uint i = node->id()%np;
        if( i > 0 )
        {
            Real & x = node->operator()(0);
            if( i < 2*nx_f ) // fluid
            {
                x = ox + h_f * (1-std::pow(f,i)) / (1-f);
            }
            else // solid
            {
                x = li + h_s * (i-2*nx_f);
...
int main (int argc, char** argv)
{
    ...
    GetPot param_file("param_fsi_tube.dat");
...
//    BInfoSystem & system_b = es_b.add_system<BInfoSystem>("binfo");
    ...
    system_vel.get_dof_map().add_dirichlet_boundary( DirichletBoundary( {6}, {u_var},  ZeroFunction<Real>() ) );
    ...
    // SOLID IN/OUT
    system_dy .get_dof_map().add_dirichlet_boundary( DirichletBoundary( {2,4}, {dy_var}, ZeroFunction<Real>() ) );
    system_vel.get_dof_map().add_dirichlet_boundary( DirichletBoundary( {2,4}, {v_var},  ZeroFunction<Real>() ) );
    ...
    es.parameters.set<Real>("p_in") = param_file("p_in", 15);
    es.parameters.set<Real>("p_out") = param_file("p_out", 13);

    es.parameters.set<std::string>("output_dir") = param_file("output_dir", "output_tube/");
    es.parameters.set<std::string>("basename") = param_file("basename", "fsitube");
                   ...
                   const std::string& system_name)
{
    ...
    ExtPressure pressure_in(es.parameters.get<Real>("p_in"));
    ExtPressure pressure_out(es.parameters.get<Real>("p_out"));
                ...
                for (uint i=0; i<n_u_dofs; i++)
                {
                    ...
                    for (uint j=0; j<n_u_dofs; j++)
                    {
                        Kuu(i,j) += JxWqp*( rho_f*phi[j][qp]*phi[i][qp]
                                            + dt*rho_f*(u_old*dphi[j][qp](0)+v_old*dphi[j][qp](1))*phi[i][qp]
                                            + dt*mu_f*(
                                                  dphi[j][qp]*dphi[i][qp]
            ...
            for (uint s=0; s<elem->n_sides(); s++)
            {
                ...
                if (elem->neighbor(s) == NULL)
                {
                    if (mesh.boundary_info->has_boundary_id(elem, s, 1) ||
                            mesh.boundary_info->has_boundary_id(elem, s, 5))
                    {
                        const std::vector<std::vector<Real> >&  phi_face = fe_face->get_phi();
                        const std::vector<Real>& JxWface = fe_face->get_JxW();
                        const std::vector<Point >& qface_point = fe_face->get_xyz();
                        ...
                        for (uint qp=0; qp<qface.n_points(); qp++)
                        {
                            ...
                            //const Real value = pressure_in(qface_point[qp], system.time);

                            const Real value = mesh.boundary_info->has_boundary_id(elem, s, 1)?
                                        pressure_in(qface_point[qp], system.time):pressure_out(qface_point[qp], system.time);
